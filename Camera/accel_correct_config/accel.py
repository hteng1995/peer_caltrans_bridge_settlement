...
def extract_meas():
    ...
    #logger.debug('Reading Config file: ' + configFile) #Commented out by Henry
    ...
    #Added by Henry:
    count_samples = 5 #num of samples we want
    count_taken = 0
    xvals = []
    yvals = []
    zvals = []
    while count_taken < count_samples:
        if runTimeConfigObject.executeSilently == 0:
            axes = MMA8451.getAxisValue()
            print("X: " + str(axes['x']) + ", Y: " + str(axes['y']) + ", Z: " + str(axes['z']))
            xvals.append(axes['x'])
            yvals.append(axes['y'])
            zvals.append(axes['z'])
